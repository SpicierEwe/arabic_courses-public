...
export default async function handler(req, res) {
  ...
  if (query.includes("course_info_id=")) {
    ...
    const [single_course_info, single_course_content] = await fetch_the_course(
      course_id
    );

    return res.status(200).json({
      course_info: single_course_info,
      status: "success",
    });
  }
  if (query.includes("course__full_data_id=")) {
    const course_id = query.split("=")[1];
    ...
    const x = {
      course_info: single_course_info,
      course_content: single_course_content,
    };
...
async function fetch_the_course(course_id) {
  let course_data = {};
  ...
  return [course_data["course_info"], course_data["course_content"]];
}
