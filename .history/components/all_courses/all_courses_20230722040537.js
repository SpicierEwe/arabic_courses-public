...
function display_stars(star, rating_number) {
  const x = [];

  function rating_number_refactor(rating_number) {
    if (isFloat(rating_number)) {
      ...
      if (b === 0.5) {
        x.push(<BsStarHalf className={styles.star} size={17} />);
      }
    }
  }

  for (let x = 0; x < rating_number; x++) {}
}
                ...
                <div className={styles.review_container}>
                  <div style={{ display: "inline-flex" }}>
                    {review_stars.map((star, index) => {
                      return display_stars(star, course_info["rating"]);
                    })}
                  </div>
