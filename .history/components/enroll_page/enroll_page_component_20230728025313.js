...
export default function EnrollPageComponent({ query }) {
  ...
  function add_to_user_enrolled_courses(selected_course_id) {
    ...
    const enrolledCoursesRef = ref(db, "users/" + userId + "/enrolled_courses");

    // Push the courseId to the enrolled_courses array.
    push(enrolledCoursesRef, courseId)
      .then(() => {
        console.log("Course added to enrolled_courses successfully!");
      })
      .catch((error) => {
        console.error("Error adding course to enrolled_courses:", error);
      });
  }
