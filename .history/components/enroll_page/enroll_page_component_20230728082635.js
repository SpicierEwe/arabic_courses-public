...
export default function EnrollPageComponent({ query }) {
  ...
  const [is_course_enrolled, setIsCourseEnrolled] = useState(false);
  const [all_enrolled_courses, set_all_enrolled_courses] = useState([]);
  // ====================================================================================================
  // ====================================================================================================
  ...
  useEffect(() => {
    fetch("/api/user/enroll/" + course_id + "?" + "user_id=" + userId, {
      method: "GET",
    }).then((response) => {
      response.json().then((data) => {
        console.log(data);
        setIsCourseEnrolled(data["result"]);
        set_all_enrolled_courses(data["enrolled_courses_list"]);
      });
    });
  }, []);
  // ====================================================================================================
  // ====================================================================================================
  //   this function adds the course to the user's enrolled courses
  function add_to_user_enrolled_courses(course_id) {
    if (!is_course_enrolled) {
      if (all_enrolled_courses.length > 0) {
        set_all_enrolled_courses((prev) => [...prev, course_id]);
      } else {
        set_all_enrolled_courses([course_id]);
      }
      fetch("/api/user/enroll/" + course_id + "?" + "user_id=" + userId, {
        method: "POST",
        body: JSON.stringify({ all_enrolled_courses }),
      });
    }
  }
        ...
        <div className={styles.hero_container_bg}>
          <section className={styles.hero_container}>
            ...
            >
              ...
              >
                {is_course_enrolled ? "Go to Course" : "Enroll"}
              </div>
              {/* </Link> */}
            </div>
