...
export default function ReviewsComponent({ course_id, isEnrolled }) {
  ...
  const submit_review = () => {
    ...
    if (is_course_reviewed) {
      ...
      const current_rating_database_ref = ref(database, previous_rating_url);

      const current_rating = my_temp_review_data.reviewer_rating;
      const current_rating_url = `courses/${course_id}/ratings/${current_rating}`;
      ...
      runTransaction(prev_rating_database_ref, (currentTotalReviews) => {
        ...
        .then((newTotalReviews) => {
          console.log("Total reviews increased to:", newTotalReviews);
        })
        .catch((error) => {
