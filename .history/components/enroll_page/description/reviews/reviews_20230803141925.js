...
export default function ReviewsComponent({ course_id }) {
      ...
      .then((reviews_snapshot) => {
        if (reviews_snapshot.exists()) {
          ...
          for (const [key, value] of Object.entries(data)) {
            user_id = value.reviewer_id;
            // reviewer_data
            const reviewer_data = get(
              child(dbRef, `users/${user_id}/user_info`)
            ).then((reviewer_data_snapshot) => {
              user_data = reviewer_data_snapshot.val();

              const t = {
                ...value,
                ...reviewer_data,
              };
              console.log("this te t = ", t);
              x.push(t);
            });
          }
          set_all_reviews(x);
  ...
  useEffect(() => {
    get(child(dbRef, `/users/${user.uid}/reviewed_courses/${course_id}/review`))
      .then((snapshot) => {
        if (snapshot.exists()) {
