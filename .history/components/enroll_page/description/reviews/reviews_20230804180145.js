...
export default function ReviewsComponent({ course_id, isEnrolled }) {
  ...
  useEffect(() => {
    ...
    const reviewsQuery = query(
      ...
      //   ordering the reviews by created_at
      orderByChild("created_at"),

      //   getting the last 4 reviews
      limitToLast(4)
    );
    ...
    onValue(reviewsQuery, (reviews_snapshot) => {
      if (reviews_snapshot.exists()) {
        ...
        const x = [];
        z.map((value, index) => {
          //   checking if the reviewer comment is not empty
  ...
  //   =============================================
  //   =============================================
  //   GETTINGS THE AVERAGE RATING and total reviews
  //   =============================================
  //   =============================================
  ...
  useEffect(() => {
    ...
    get(child(dbRef, `courses/${course_id}/total_reviews`)).then((snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
  ...
  const submit_review = () => {
    ...
    if (is_course_reviewed) {
      //  if course is already reviewed then update the review

      // update review
      const new_global_review_format_updated_review = {
        [my_review_data.review_id]: {
          // review id
          review_id: my_review_data.review_id,
          reviewer_rating: my_temp_review_data.reviewer_rating,
          reviewer_comment: my_temp_review_data.reviewer_comment,
          reviewer_id: user.uid,
          created_at: my_review_data.created_at,
      ...
      update(
        ...
        new_global_review_format_updated_review
      );
      ...
      set_my_review_data(
        new_global_review_format_updated_review[my_review_data.review_id]
      );

      //   =============================================
      //   =============================================
      //   =============================================
      //   update global raiting of the course
      const database = getDatabase();
      //   this is the prev rating url in the database which is to be updated by decreasing the total reviews by 1
      const previous_rating_url = `courses/${course_id}/ratings/${my_review_data.reviewer_rating}`;

      const prev_rating_database_ref = ref(database, previous_rating_url);
      //

      const new_rating = my_temp_review_data.reviewer_rating;
      const new_rating_url = `courses/${course_id}/ratings/${new_rating}`;
      const new_rating_database_ref = ref(database, new_rating_url);

      //   updating the old rating by decreasing the total rating by 1
      runTransaction(prev_rating_database_ref, (currentTotalReviews) => {
        // If currentTotalReviews is null, it means the field doesn't exist, so initialize it to 1

        if (currentTotalReviews != 0) {
          return currentTotalReviews - 1;
        }
        return;
      })
        .then(() => {
          runTransaction(new_rating_database_ref, (currentTotalReviews) => {
            // If currentTotalReviews is null, it means the field doesn't exist, so initialize it to 1
            return (currentTotalReviews || 0) + 1;
          });
        })

        .catch((error) => {
          console.error("Error increasing total reviews:", error);
        });
      ...
      runTransaction(new_rating_database_ref, (currentTotalReviews) => {
        ...
        .then(() => {
          const total_reviews_url = `courses/${course_id}/total_reviews`;
          const total_reviews_database_ref = ref(database, total_reviews_url);
          runTransaction(total_reviews_database_ref, (currentTotalReviews) => {
            // If currentTotalReviews is null, it means the field doesn't exist, so initialize it to 1
            return (currentTotalReviews || 0) + 1;
          });
        })
        ...
        <main>
          <div className={styles.entire_review_container}>
            <section className={styles.left_rating_container}>
              ...
              <div className={styles.global_rating_parent_count_container}>
                <div className={styles.global_rating_count_container}>
                  <AiFillStar className={styles.star_icon}></AiFillStar>
                  <p className={styles.global_rating_count}>{averageRating}</p>
                </div>
                <p className={styles.total_reviews}>
                  T-REVIEWS : {totalReviews != NaN ? totalReviews : 0}
                </p>
              </div>
              ...
              {isEnrolled ? (
                <div className={styles.my_rating_container}>
                  ...
                  {/* only show my review comment when its not empty */}
                    ...
                    return (
                      <div key={index} className={styles.flex}>
                        ...
                        <div className={styles.review_container_c2}>
                          ...
                          <p className={styles.created_date_style}>
        ...
        <div className={styles.my_post_review_container}>
          ...
          <div className={styles.main_content}>
            ...
            <div
              className={styles.edit_review_button}
              onClick={() => {
                if (my_temp_review_data.reviewer_rating != 0) {
                  if (
                    my_temp_review_data.reviewer_comment !=
                      my_review_data.reviewer_comment ||
                    my_temp_review_data.reviewer_rating !=
                      my_review_data.reviewer_rating
                  ) {
