...
export default function ReviewsComponent({ course_id, isEnrolled }) {
  ...
  const submit_review = () => {
    ...
    if (is_course_reviewed) {
      ...
      const link = `courses/${course_id}/ratings/${my_temp_review_data.reviewer_rating}`;

      // Assuming you have already initialized Firebase and have a reference to the database
      const database = getDatabase();

      const course_id = "your_course_id"; // Replace with the actual course ID

      // Increase total reviews by 1
      const courseRef = ref(database, `courses/${course_id}/total_reviews`);
