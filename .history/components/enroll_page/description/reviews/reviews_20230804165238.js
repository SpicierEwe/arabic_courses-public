...
export default function ReviewsComponent({ course_id, isEnrolled }) {
  ...
  const submit_review = () => {
    ...
    if (is_course_reviewed) {
      ...
      const current_raiting_url = `courses/${course_id}/ratings/${my_review_data.reviewer_rating}`;

      // Assuming you have already initialized Firebase and have a reference to the database
      const database = getDatabase();
      const courseRef = ref(database, current_raiting_url);

      const current_rating = my_review_data.reviewer_rating;
