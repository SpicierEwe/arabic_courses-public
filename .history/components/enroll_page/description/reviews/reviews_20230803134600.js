...
export default function ReviewsComponent({ course_id }) {
  ...
  const [my_review_data, set_my_review_data] = useState({
    raiting: 4,
    comment: "This course is all that you need.",
  });

  useEffect(() => {
    const dbRef = ref(getDatabase());
    get(child(dbRef, `/users/${user.uid}/reviewed_courses/${course_id}`))
      .then((snapshot) => {
        if (snapshot.exists()) {
  ...
  const x = () => {
    ...
    update(
      child(dbRef2, `/users/${user.uid}/reviewed_courses/${course_id}/review`),
