...
export default function EnrollPageComponent({ query }) {
  ...
  function add_to_user_enrolled_courses(selected_course_id) {
    ...
    // Get a key for the new course.
    const newCourseKey = push(child(ref(db), "courses")).key;

    // Write the new course's data simultaneously in the courses list and the user's enrolled_courses list.
    const updates = {};
    updates["/courses/" + newCourseKey] = courseId;
    updates["/users/" + uid + "/enrolled_courses/" + newCourseKey] = courseId;
