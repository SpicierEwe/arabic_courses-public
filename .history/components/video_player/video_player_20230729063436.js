          ...
          .then((snapshot) => {
            if (snapshot.exists()) {
              ...
              if (!snapshot.val().includes(videoId)) {
                // if the video is not already in the completed videos list
                // we will add it to the list
                const newCompletedVideosList = [...snapshot.val(), videoId];
                set(
                  ref(getDatabase(), `users/${userId}/completed_videos`),
                  newCompletedVideosList
                );
              }
            } else {
