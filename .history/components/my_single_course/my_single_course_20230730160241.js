...
export default function MySingleCourseDisplayComponent({ course_id }) {
  ...
  //  this use effect makes the selected video to scroll into view when the page is loaded or a new video is selected
  useEffect(() => {
    const lastWatchedVideo = JSON.parse(
      localStorage.getItem("last_watched_video")
    );
    ...
    if (lastWatchedVideo) {
      // const last_watched_video_id = lastWatchedVideo[0]["video_id"];
      // const targetItemRef = video_refs.current[last_watched_video_id];
      // if (targetItemRef) {
      ...
      const dvd_id = `B${lastWatchedVideo[0]["dvd_number"]}D${lastWatchedVideo[0]["video_number"]}`;
