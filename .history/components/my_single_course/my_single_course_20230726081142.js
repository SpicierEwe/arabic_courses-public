...
export default function MySingleCourseDisplayComponent({ course_data }) {
  ...
  const [nav_bar_height, setNavBarHeight] = useState(null);

  useEffect(() => {
    if (ctx.nav_bar_ref) {
      const nav_bar_ref = ctx.nav_bar_ref;
      const navbarHeight = nav_bar_ref.current.offsetHeight;
      const viewportHeight = window.innerHeight;
      //   const nav_bar_height_in_vh = (100 * navbarHeight) / window.innerHeight;
      const nav_bar_height_in_vh = (navbarHeight / viewportHeight) * 100;
      setNavBarHeight(nav_bar_height_in_vh);

      //   console.log("Navbar height:", nav_bar_height_in_vh + "vh");
  ...
  return (
    <div>
      ...
      <main
        ...
        style={{
          paddingTop: `${ctx.nav_bar_height}vh`,
        }}
      >
        {/* side_nav */}
        <SideNavComponent
