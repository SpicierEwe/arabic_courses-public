      ...
      const initial_data = [
        {
          ...
          video_url:
            "https://joy1.videvo.net/videvo_files/video/free/2012-10/large_watermarked/hd1967_preview.mp4",
        },
      ];

      if (selected_video_data.length == 0 && ctx.videoPlayerRef != null) {
        setSelectedVideoData(initial_data);
        if (ctx.videoPlayerRef != null) {
          console.log("single course screen the ref is : ", ctx.videoPlayerRef);
          ctx.videoPlayerRef.current.load();
          ctx.videoPlayerRef.current.play();
        }
      }
    }

    x();
  }, [ctx.videoPlayerRef, selected_video_data]);

  // useEffect(() => {
  //   console.log("the video player ref is: ", ctx.videoPlayerRef);
  // }, [ctx.videoPlayerRef]);
  return (
    <div>
      {/* {!ctx.isNotified && (
