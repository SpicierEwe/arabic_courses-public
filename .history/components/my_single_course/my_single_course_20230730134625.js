...
export default function MySingleCourseDisplayComponent({ course_id }) {
  ...
  useEffect(() => {
    const scrollToElement = () => {
      // The "to" parameter can be the name of the element you want to scroll to (if using Element component) or a numeric value representing the position to scroll to.
      scroll.scrollTo("myElement", {
        smooth: true,
        duration: 500, // Optional: Duration of the scroll animation in milliseconds
        offset: -50, // Optional: Offset from the top of the element. Useful if you want to adjust the scroll position.
      });
    };
