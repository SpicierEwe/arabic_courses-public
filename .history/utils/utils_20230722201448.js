...
export function display_stars_function(rating_number) {
  const icon_size = 15;
  const star_style = { marginRight: "2px", color: "#0077b6" };
  const filled_star = <BsStarFill style={t} size={icon_size} />;
  const half_star = <BsStarHalf style={t} size={icon_size} />;
  const empty_star = <BsStar style={t} size={icon_size} />;
