...
export function display_stars_function(rating_number) {
  ...
  const filled_star = (
    <BsStarFill style={star_style} size={icon_size} key={uniqueId1} />
  );
  const half_star = (
    <BsStarHalf style={star_style} size={icon_size} key={uniqueId2} />
  );
  const empty_star = (
    <BsStar style={star_style} size={icon_size} key={uniqueId3} />
  );
