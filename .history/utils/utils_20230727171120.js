...
export function display_stars_function(rating_number) {
  ...
  const filled_star = (
    <BsStarFill style={star_style} size={icon_size} key={13552 + uniqueId} />
  );
  const half_star = (
    <BsStarHalf style={star_style} size={icon_size} key={22342 + uniqueId} />
  );
  const empty_star = (
    <BsStar style={star_style} size={icon_size} key={34343 + uniqueId} />
  );
